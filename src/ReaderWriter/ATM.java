...
public class ATM {
	...
	//implements Serializable {
		...
		// vraæa ukupnu vrijednost novca u bankomatu
		int balance (){
			int sum=0;
			for (int i=0; i<4; i++){
		...
		// dodavanje novèanica od strane admnistratora
		boolean deposite(int bill, int amount){
			if (amount > (100-banknote[1][bill])){
				System.out.println("U odjeljku za novcanice od "+ banknote[0][bill] +" KM može stati još " + (100 - banknote[1][bill]) + " novcanica.");
				System.out.println("Ne možete dodati "+amount+" novcanica.");
				return false;
			} else {
				banknote[1][bill] += amount;
				System.out.println("Sada u odjeljku za novcanice od "+ banknote[0][bill] +" KM imate " + banknote[1][bill] + " novcanica.");
				return true;
			}
		}
		
		// provjerava da li je korisniku moguæe isplatiti trazeni iznos
		boolean isPosiblePersonWithdraw(int amount){
			return (balance()>=amount);
		}
		
		// uzimanje novèanica odreðene vrijednosti od strane administratora
		boolean withdraw(int bill, int amount){
			if (amount > banknote[1][bill]){
				System.out.println("U odjeljku za novcanice od "+ banknote[0][bill] +" KM imate " + banknote[1][bill] + " novcanica.");
		...
		void status(){
			for (int i=0; i<4; i++){
				System.out.println("U odjeljku za novèanice od "+ banknote[0][i] +" KM imate " + banknote[1][i] + " novèanica.");
			}
			System.out.println("\n");
		...
		void showMenu(){
			...
			this.status(); // prikazuje stanje u bankomatu
